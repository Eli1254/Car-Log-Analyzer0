...
def plot_sensor(data, sensor, highlight_events=False, metric=None):
...
def plot_boost_vs_rpm(data, window, poly):
    required_cols = ['RPM (RPM)', 'Boost (psi)']
    if not all(col in data.columns for col in required_cols):
        st.warning("Required columns missing for Boost vs RPM.")
        return

    boost = savgol_filter(data['Boost (psi)'], window_length=window, polyorder=poly)
    plt.figure(figsize=(12, 6))
...
def plot_torque_vs_rpm(data, window, poly):
    required_cols = ['RPM (RPM)', 'Req Torque (Nm)']
    if not all(col in data.columns for col in required_cols):
        st.warning("Required columns missing for Torque vs RPM.")
        return

    torque = savgol_filter(data['Req Torque (Nm)'], window_length=window, polyorder=poly)
    plt.figure(figsize=(12, 6))
...
def plot_boost_vs_torque(data, window, poly):
